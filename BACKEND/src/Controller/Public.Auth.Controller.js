...
const PublicAuthController = {
    ...
    signUp: asyncHandler(async (req, res) => {
        ...
        if (isUserExist || MobileExist) {
            res.status(409).json({ status: false, errorMessage: 'User Already Exist' })
        } else {
    ...
    changeUserPassword: asyncHandler(async (req, res) => {
        ...
        if (usrDetail.oldpassword !== usrDetail.npassword) {
            ...
            res.status(401).json({
                status: false,
                errorMessage: 'you old password has been matched Please enter another password '
            })
        }
    }),
    ...
    passwordReset: asyncHandler(async (req, res) => {
        ...
        await jwt.verify(checkPassword.resetoken, process.env.SECRET_KEY, async (err) => {
            if (err) {
