...
import { UPDATE_PROFILE_SAGA } from "../../../../../Stores/Slice/Seller.Auth.Slice";
...
const SellerProfile = () => {
  ...
  useEffect(() => {
    if (profile?.avatar?.status === true) {
      setUpdateProfile(false);
    }
  }, [profile?.avatar?.status, setUpdateProfile]);
        ...
        <div className="col-span-12 mt-6 divide-y max-sm:flex-col max-sm:items-center md:col-span-6">
          ...
          <div className="pt-3 mt-3">
            <div>
              ...
              <div className="mt-2">
                {user.user?.fullAddress?.address} {user.user?.fullAddress?.city}
                ,
              </div>
              <div>
                {user.user?.fullAddress?.state}{" "}
                {user.user?.fullAddress?.pincode}
              </div>
              <div>{user.user?.fullAddress?.country}</div>
            </div>
          </div>
          <div className="pt-3 mt-3">
            <div className="">
              ...
              <div className="flex flex-col gap-1 mt-2">
                <div>Company : {user.user?.bussinessDetail?.company}</div>
                <div>GSTIN : {user.user?.gstNum}</div>
              </div>
              <div className="mt-3 ">
                <div className="font-bold text-daintree-400 text-1xl">
                  Bussiness Address
                ...
                <div>
                  <div className="mt-2">
                    {user.user?.bussinessDetail?.companyAddress?.address}{" "}
                    {user.user?.bussinessDetail?.companyAddress?.city},
                  </div>
                  <div>
                    {user.user?.bussinessDetail?.companyAddress?.state}{" "}
                    {user.user?.bussinessDetail?.companyAddress?.pincode}
                  </div>
                  <div>
                    {user.user?.bussinessDetail?.companyAddress?.country}
                  </div>
                </div>
              </div>
            </div>
